...
* **H2 (exploratory generalization-gap rule).** `in_distribution_accuracy` uses only example-disjoint dev data (lengths 4–8); training accuracy is excluded. The statistic is `mean_seeds[(dev - heldout)(baseline) - (dev - heldout)(looped)]`. The ±0.05 threshold is operational only. A smaller gap can result from worse dev accuracy, so meeting it does not establish improved generalization. Always show absolute dev and held-out performance. Historical artifacts retain their original pooled train+dev definition; do not combine those H2 values with this revision.
