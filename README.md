...
- **H1, H2 and H0 are all computed in code** and marked `evaluated` in the report. H2 is `drop(baseline) - drop(looped)`, where `drop = in_distribution_accuracy(train+dev, lengths 4–8) - heldout_accuracy(lengths 12–16)`.
