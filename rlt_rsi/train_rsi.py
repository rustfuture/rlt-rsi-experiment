...
def write_report(path: Path, payload: Dict[str, object]) -> None:
    ...
    lines.extend([
        "1. **RSI adaptation vs Fixed RLT vs Baseline.** "
...
def main(argv: Iterable[str] | None = None) -> int:
    ...
    ap.add_argument("--rsi-generations", type=int, default=5, help="Number of RSI generations")
    ap.add_argument("--rsi-epochs-per-gen", type=int, default=16, help="Epochs per generation")
    ...
    output: Path = args.output or (artifacts_dir / "run.json" if artifacts_dir else Path("results/rsi_smoke.json"))
    ...
    for s in seeds:
        ...
        for model, loops in EXPERIMENT_CONFIGS:
            ...
            if model == "rsi_looped":
                generations = args.rsi_generations
                epochs_per_gen = args.rsi_epochs_per_gen
                if backend == "numpy":
                    metrics = run_rsi_numpy(
                        splits, cfg, generations=generations, epochs_per_gen=epochs_per_gen,
                        learning_rate=learning_rate, weight_decay=1e-4, seed=s
                    )
                else:
                    metrics = run_rsi_torch(
                        splits, cfg, generations=generations, epochs_per_gen=epochs_per_gen,
                        learning_rate=learning_rate, weight_decay=weight_decay, seed=s, device=torch_device
                    )
            else:
                if backend == "numpy":
                    metrics = run_numpy(
